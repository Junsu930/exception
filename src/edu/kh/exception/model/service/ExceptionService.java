...
public class ExceptionService {
	...
	// 에외(Exception) : 소스코드의 수정으로 해결 가능한 오류
	...
	public void ex1() {
		...
		// catch(예외){ } : try 구문에서 발생한 예외를 잡아내어 처리하여,
	...
	public void ex3() {
		...
		// * NullPointerException : 참조하지 않는 참조변수를 이용해서 코드 수행할 때 발생
		
		String str = null;
		System.out.println(str.charAt(0)); 
		
		// 관계없는 예외는 순서 관계 없이 catch문에 작성하면 된다. 
		}catch(ArithmeticException e) {
			...
			System.out.println("정확하게 알 수 없으나, 예외가 발생하여 처리함");
		}
	...
	public void ex4() {
		...
		try {
			System.out.println(4/0); // ArithmeticException 발생시킴
	...
	public void ex5() { 
		...
		try{
			...
			// Exception e : 모든 예외의 최상위 부고
			// -> Exception이 catch 매개변수로 작성되었다. == 예외 종류 상관없이
			//   모두 처리
	...
	public void methodD() {
		...
		// CheckedException :
		// RuntimeExcption과 자식 에외를 제외한 나머지 예외.
		// -if문과 같은 단순 코드로 해결이 불가능하며
		// 반드시 예외처리 구문을 작성해야 하는 예외
		
		// UnchecekdException :
		// RuntimeException 과 자식 예외를 지칭함
		// -프로그램 수행 중 개발자의 코딩 실수 또는
		// 사용자의 잘못된 값 입력으로 흔하게 발생
